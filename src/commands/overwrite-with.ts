...
const overwriteWithFn = async (context: CNDIContext, initializing = false) => {
  ...
  const leaders = entries.filter((entry: BaseNodeEntrySpec) => entry.role === "leader");
  ...
  entries
    .sort((nodeItemA, nodeItemB) => {
      return nodeItemA.name.localeCompare(nodeItemB.name);
    })
    .forEach((entry: BaseNodeEntrySpec, nodeIndex: number) => {
