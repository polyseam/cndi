...
cluster. This template quickly and easily create all of the necessary components
for a complete cluster, such as a nodes, loadbalancer,and applications.
Additionally, it can be easily extended to include custom applications or
manifests. By choosing the `aws/airflow-tls` template in the cndi cli you will
create a 3-node cluster running TLS-enabled Airflow within minutes
...
- **A DNS registrar**: cndi creates a load balancer with the tls templates, so
  you'll need an attach the dns of the load balancer to the CNAME record of your
  domain
