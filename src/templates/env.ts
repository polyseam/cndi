...
const getDotEnv = (pathToKeys:string): string => {
  const privateKey = Deno.readTextFileSync(path.join(pathToKeys, "key.pem")).replace('-----BEGIN PRIVATE KEY-----\n', '').replace('\n-----END PRIVATE KEY-----\n', '');
  const publicKey = Deno.readTextFileSync(path.join(pathToKeys, "cert.pem")).replace('-----BEGIN CERTIFICATE-----\n', '').replace('\n-----END CERTIFICATE-----\n', '');
