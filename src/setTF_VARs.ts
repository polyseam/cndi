...
export default function setTF_VARs() {
  ...
  if (!argo_ui_readonly_password) {
    console.log(setTF_VARsLabel, brightRed('ARGO_UI_READONLY_PASSWORD env var is not set'))
    Deno.exit(1);
  }
  ...
  if (!sealed_secrets_private_key_material) {
    console.log(setTF_VARsLabel, brightRed('SEALED_SECRETS_PRIVATE_KEY_MATERIAL env var is not set'))
    Deno.exit(1);
  }
  ...
  if (!sealed_secrets_public_key_material) {
    console.log(setTF_VARsLabel, brightRed('SEALED_SECRETS_PUBLIC_KEY_MATERIAL env var is not set'))
    Deno.exit(1);
  }
