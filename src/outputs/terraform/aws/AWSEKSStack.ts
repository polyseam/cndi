import { CNDIConfig, CNDINodeSpec, TFBlocks } from "src/types.ts";
...
export default class AWSEKSTerraformStack extends AWSCoreTerraformStack {
  constructor(scope: Construct, name: string, cndi_config: CNDIConfig) {
    ...
    if (autoMode) {
      const availabilityZones = new CDKTFProviderAWS.dataAwsAvailabilityZones.DataAwsAvailabilityZones(this, "available-zones", {
        filter: [
          {
            name: "opt-in-status",
      ...
      if(network.mode === "create") {
        const vpc = new AwsVpcModule(this, "cndi_aws_vpc_module", {
        ...
        vpc_id = vpc.vpcIdOutput;
        subnetIds = vpc.privateSubnetsOutput;
      } else {
        // (network.mode === "insert") 
        vpc_id = network.vnet_identifier!;
      }
      ...
      const ebsCsiPolicy = new CDKTFProviderAWS.dataAwsIamPolicy
        .DataAwsIamPolicy(
        this,
        "ebs_csi_policy",
        {
      ...
      const efsCsiPolicy = new CDKTFProviderAWS.dataAwsIamPolicy
        .DataAwsIamPolicy(
        this,
        "efs_csi_policy",
        {
      ...
      const _efsAccessPoint = new CDKTFProviderAWS.efsAccessPoint
        .EfsAccessPoint(
        this,
      ...
      for (
        const nodeGroup of cndi_config.infrastructure.cndi
          .nodes as CNDINodeSpec[]
      ) {
        ...
        eksManagedNodeGroups[nodeGroupName] = new CDKTFProviderAWS
          .eksNodeGroup
          .EksNodeGroup(
          this,
