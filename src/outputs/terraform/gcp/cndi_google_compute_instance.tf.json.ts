...
export default function getGCPComputeInstanceTFJSON(
  ...
  const machine_type = node?.machine_type || node?.instance_type || DEFAULT_MACHINE_TYPE;
  ...
  const network = "${google_compute_network.cndi_google_compute_network.self_link}"; //network to attach this interface to.
  const subnetwork = "${google_compute_subnetwork.cndi_google_compute_subnetwork.self_link}"; //subnetwork to attach this interface to.
